...
final class SnakeGameEngine {
    ...
    private(set) var dailyChallenge: DailyChallengeDefinition?
    ...
    private(set) var isManualStepModeEnabled = false
    ...
    private var activeDeliveryRoute: DeliveryRouteDefinition?
    private var deliveryStationPoint: GridPoint?
    ...
    var snapshot: GameSnapshot {
        let deliveryStation = makeDeliveryStationSnapshot()
        return GameSnapshot(
            level: level,
            modifier: modifier,
            mission: mission,
            ...
            isManualStepModeEnabled: isManualStepModeEnabled,
    ...
    func restart(
        ...
        temporaryHazards = []
        activeDeliveryRoute = nil
        deliveryStationPoint = nil
        ...
        configureDeliveryRouteIfNeeded()
        fruitPosition = nil
        fruit = nil
        spawnFruit()
    ...
    var canActivateBoost: Bool {
        !isGameOver && !isManualStepModeEnabled && snake.count > 3
    }
    ...
    func advance() -> [GameEvent] {
        ...
        if let stationPoint = deliveryStationPoint,
           !deliveryCompleted,
           deliveryReadyKinds.count == deliveryTargetKinds.count,
           nextHead == stationPoint {
            deliveryCompleted = true
            runStats.deliveriesCompleted += 1
            if isSimpleModeEnabled {
                childSafetyBrakeAvailable = true
                slowMovesRemaining = max(slowMovesRemaining, 4)
            }
        }
    ...
    private func spawnFruit() {
        let occupied = Set(snake)
            ...
            .union(deliveryStationPoint.map { [$0] } ?? [])
    ...
    private static func makeMissionProgress(
        ...
        case .deliverRoute(let route):
            let collected = Set(route.requiredKinds).filter { runStats.fruits(for: $0) > 0 }.count
            current = deliveryCompletedValue(collected: collected, delivered: snapshot?.deliveryStation?.isCompleted ?? false)
        case .specialFruit:
    ...
    private var currentGameplayHint: String {
        ...
        if let delivery = makeDeliveryStationSnapshot() {
            if delivery.isCompleted {
                deliveryHint = "；送货完成"
            } else if delivery.isReadyForDelivery {
                deliveryHint = "；把列车开到\(delivery.route.station.title)"
            } else {
                deliveryHint = "；送货目标：\(delivery.route.station.title)"
            }
        } else {
            deliveryHint = ""
        }
        if isSimpleModeEnabled {
            ...
            let movementHint = isManualStepModeEnabled
                ? "；每按一次方向键或手柄方向，火车才会前进一步"
                : "；儿童模式节奏更慢"
            let boostHint = isManualStepModeEnabled ? "" : "；长按空格喷射，每 5 秒一次，会甩掉一节尾巴"
            return gameplayHint + movementHint + "，火车会按水果种类增加车厢" + boostHint + brakeHint + deliveryHint
        }
    ...
    private func configureDeliveryRouteIfNeeded() {
        guard case .deliverRoute(let route) = mission.goal else {
            return
        }
        activeDeliveryRoute = route
        deliveryStationPoint = resolvedPoint(for: route.station.anchor)
    }

    private func resolvedPoint(for anchor: DeliveryStationAnchor) -> GridPoint? {
        let candidates: [GridPoint]
        switch anchor {
        case .top:
            candidates = (0 ..< level.columns).map { GridPoint(x: $0, y: level.rows - 1) }
        case .right:
            candidates = (0 ..< level.rows).map { GridPoint(x: level.columns - 1, y: $0) }
        case .bottom:
            candidates = (0 ..< level.columns).map { GridPoint(x: $0, y: 0) }
        case .left:
            candidates = (0 ..< level.rows).map { GridPoint(x: 0, y: $0) }
        }

        let dynamicBlocked = Set(dynamicObstacle?.points ?? [])
        let sorted = candidates.sorted {
            let lhsDistance = abs($0.x - level.columns / 2) + abs($0.y - level.rows / 2)
            let rhsDistance = abs($1.x - level.columns / 2) + abs($1.y - level.rows / 2)
            return lhsDistance < rhsDistance
        }
        return sorted.first { point in
            !level.obstacles.contains(point) && !dynamicBlocked.contains(point)
        }
    }

    private var deliveryTargetKinds: Set<FruitKind> {
        guard let route = activeDeliveryRoute else {
            return []
        }
        return Set(route.requiredKinds)
    }

    private var deliveryReadyKinds: Set<FruitKind> {
        deliveryTargetKinds.filter { runStats.fruits(for: $0) > 0 }
    }

    private func makeDeliveryStationSnapshot() -> DeliveryStationSnapshot? {
        guard let route = activeDeliveryRoute, let point = deliveryStationPoint else {
            return nil
        }
        let collectedKinds = route.requiredKinds.filter { runStats.fruits(for: $0) > 0 }
        return DeliveryStationSnapshot(
            route: route,
            point: point,
            collectedKinds: collectedKinds,
            isReadyForDelivery: Set(collectedKinds).count == Set(route.requiredKinds).count,
            isCompleted: deliveryCompleted
        )
    }

    private static func deliveryCompletedValue(collected: Int, delivered: Bool) -> Int {
        delivered ? collected + 1 : collected
    }
}
